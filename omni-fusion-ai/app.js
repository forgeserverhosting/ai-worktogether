...
const STORAGE_KEY = 'omnifusion-studio-v14';
const LEGACY_STORAGE_KEYS = ['omnifusion-studio-v13', 'omnifusion-studio-v12', 'omnifusion-studio-v11', 'omnifusion-studio-v10', 'omnifusion-studio-v9'];
...
const els = {
  ...
  changesView: $('#changesView'), changeCountBadge: $('#changeCountBadge'), validationView: $('#validationView'), brandMemoryView: $('#brandMemoryView'), versionsView: $('#versionsView'), intelligenceView: $('#intelligenceView'), projectBrainView: $('#projectBrainView'), brainStatusBadge: $('#brainStatusBadge'), requestPlanView: $('#requestPlanView'), requestPlanBadge: $('#requestPlanBadge'), routerLearningView: $('#routerLearningView'), routerLearningBadge: $('#routerLearningBadge'), browserTestsView: $('#browserTestsView'), browserTestBadge: $('#browserTestBadge'), benchmarkView: $('#benchmarkView'), runBenchmarkBtn: $('#runBenchmarkBtn'),
...
const roleDefinitions = [
  {
    role: 'Prime Lead',
    instruction: 'Act as the senior strategist, conversion copy lead, creative director, and UX architect. Read every verified fact, user preference, image finding, and selected design concept. Produce one decisive implementation brief that resolves ambiguity, includes exact copy direction, section order, mobile behavior, factual guardrails, distinctive design decisions, and measurable acceptance criteria. Ask no new questions because the chat intake is already complete.',
    deliverable: 'A unified website blueprint containing the content system, design DNA, responsive UX, factual guardrails, file requirements, and developer acceptance checklist.'
  }
];
...
async function startWebsiteIntake(info, attachments = []) {
  ...
  project.memory = { verifiedFacts: info ? [info] : [], preferences: {}, aiSuggestions: [], assumptions: [] };
  project.intake = {
    businessInfo: info,
    step: 0,
    ...
    answers: {
      ...
      ...inferKnownWebsiteAnswers(info)
    },
    concepts: [],
    selectedConceptId: null,
    questionPlan: adaptiveQuestionPlan(info, { siteType: inferSiteType(info), ...inferKnownWebsiteAnswers(info) }),
    messages: [
      ...
      intakeMessage('assistant', project.attachments.length ? `I received ${project.attachments.length} picture${project.attachments.length === 1 ? '' : 's'}. A vision specialist will inspect them once, save the findings, and then I’ll ask only the missing decisions here in chat.` : `Got it. I found ${Object.keys(inferKnownWebsiteAnswers(info)).length + 1} useful decisions in your message. I’ll ask only ${adaptiveQuestionPlan(info, { siteType: inferSiteType(info), ...inferKnownWebsiteAnswers(info) }).length} missing question${adaptiveQuestionPlan(info, { siteType: inferSiteType(info), ...inferKnownWebsiteAnswers(info) }).length === 1 ? '' : 's'} before building.`)
    ]
  };
  ...
  else if (!currentIntakeQuestion(project)) await generateConversationalConcepts(project);
  return project;
}
...
function deriveBrandMemory(project = activeProject()) {
  ...
  const inferredName = nameLine?.split(':').slice(1).join(':').trim()
...
Every teammate must preserve this direction, avoid generic template patterns, and use only supported business facts.`, { model: 'user-guided Website Architect' }));
  ...
  if (!stageComplete(project, 'build-files') || !project.artifact?.files?.length) {
    ...
    const build = await postProjectStep({ action: 'build', mode: project.mode, prompt: project.prompt, outputFormat: state.settings.outputFormat, modelIds: builderModelIds, slot: 0, workspace: project.teamMessages, projectName: project.title, profile: project.websiteProfile, concept: project.designConcept, memoryContext: memoryContextForRole(project, 'Frontend Developer') }, 50000, 2);
...
async function loadStatus() {
  try {
    ...
    state.status = data; document.title = data.appName || 'OmniFusion Prime Intelligence V14';
