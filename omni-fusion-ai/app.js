...
const STORAGE_KEY = 'omnifusion-studio-v8';
...
const els = {
  ...
  teamPanel: $('.team-panel'), teamStatus: $('#teamStatus'), teamSummary: $('#teamSummary'), teamFeed: $('#teamFeed'), agentChatForm: $('#agentChatForm'), agentSelect: $('#agentSelect'), agentPrompt: $('#agentPrompt'), manualCollabBar: $('#manualCollabBar'), manualStepLabel: $('#manualStepLabel'), manualDirectionInput: $('#manualDirectionInput'), nextAgentBtn: $('#nextAgentBtn'), autoContinueBtn: $('#autoContinueBtn'), openTranscriptBtn: $('#openTranscriptBtn'), transcriptModal: $('#transcriptModal'), closeTranscriptBtn: $('#closeTranscriptBtn'), transcriptStats: $('#transcriptStats'), transcriptList: $('#transcriptList'), copyTranscriptBtn: $('#copyTranscriptBtn'),
...
function syncSettingsUi() {
  ...
  els.promptModeLabel.textContent = `Website Studio · ${state.settings.collaborationMode === 'manual' ? 'manual handoffs' : 'auto collaboration'} · ${state.settings.reviewRounds} review round${state.settings.reviewRounds === 1 ? '' : 's'}`;
}
...
async function runBuild(event) {
  ...
  showError(''); state.running = true; els.runBtn.disabled = true; els.runBtn.querySelector('span').textContent = 'Building website…'; els.runProgress.classList.remove('hidden');
  ...
  } finally { state.running = false; els.runBtn.disabled = false; els.runBtn.querySelector('span').textContent = 'Build website'; updateProjectHeader(project); }
}
...
els.newProjectBtn.addEventListener('click', () => { createProject(); els.promptInput.placeholder = 'Describe any project, analysis, document, or software task…'; els.promptInput.focus(); });
els.websiteArchitectBtn.addEventListener('click', () => openArchitect());
els.emptyArchitectBtn.addEventListener('click', () => openArchitect());
els.architectChip.addEventListener('click', () => openArchitect());
els.emptyGeneralBtn.addEventListener('click', () => { els.promptInput.placeholder = 'Describe any project, analysis, document, or software task…'; els.promptInput.focus(); });
