...
const STORAGE_KEY = 'omnifusion-studio-v10';
...
const els = {
  ...
  buildForm: $('#buildForm'), promptInput: $('#promptInput'), promptCount: $('#promptCount'), promptModeLabel: $('#promptModeLabel'), runBtn: $('#runBtn'), attachImageBtn: $('#attachImageBtn'), imageInput: $('#imageInput'), attachmentStrip: $('#attachmentStrip'),
...
async function fileToCompressedAttachment(file) {
  ...
  return { id: uid('image'), name: file.name.slice(0, 100), type, width, height, dataUrl, path: '' };
}
...
function renderPendingAttachments() {
  ...
  els.attachImageBtn.textContent = items.length ? `＋ ${items.length} picture${items.length === 1 ? '' : 's'}` : '＋ Picture';
  els.attachmentStrip.innerHTML = items.map((item, index) => `<div class="attachment-card"><img src="${escapeHtml(item.dataUrl)}" alt="${escapeHtml(item.name)}"><small>${escapeHtml(item.name)}</small><button type="button" data-remove-attachment="${index}" aria-label="Remove ${escapeHtml(item.name)}">×</button></div>`).join('');
...
async function addImageFiles(files) {
  ...
  try {
    for (const file of selected) state.pendingAttachments.push(await fileToCompressedAttachment(file));
    showError('');
  } catch (error) { showError(error.message); }
  finally { els.attachImageBtn.disabled = false; els.imageInput.value = ''; renderPendingAttachments(); }
}
...
async function postProjectStep(payload, timeoutMs = 52000, retries = 1) {
  ...
  for (let retry = 0; retry <= retries; retry += 1) {
    try {
      const data = await postJson('/api/project-step', { ...payload, slot: Number(payload.slot || 0) + retry }, timeoutMs);
...
async function startWebsiteIntake(info, attachments = []) {
  ...
  project.title = intakeProjectTitle(info || attachments[0]?.name || 'New website');
  ...
  project.intake = {
    businessInfo: info,
    step: 0,
    stage: project.attachments.length ? 'scanning-images' : 'questions',
    answers: {
    ...
    messages: [
      intakeMessage('user', info || 'Use the attached pictures to create the website.', { attachmentIds: project.attachments.map((item) => item.id) }),
      intakeMessage('assistant', project.attachments.length ? `I received ${project.attachments.length} picture${project.attachments.length === 1 ? '' : 's'}. Two vision teammates will inspect them, then I’ll ask a few short questions here in the chat.` : 'Got it. I’ll use those facts and ask only a few creative questions before the team starts building.')
    ]
  };
...
async function scanProjectImages(project = activeProject()) {
  ...
  const images = attachments.map((item) => ({ id: item.id, name: item.name, type: item.type, dataUrl: item.dataUrl }));
  const modelIds = workflowModelIds(6);
  const analyses = [];
  const stages = [
    {
      role: 'Vision Analyst',
      instruction: 'Inspect every uploaded picture carefully. Identify visible subjects, text, branding, colors, composition, quality problems, useful business facts, and how each image could be used in a website. Separate visible facts from uncertain interpretations.'
    },
    {
      role: 'Visual Brand Reviewer',
      instruction: 'Review the uploaded pictures and the first analyst’s findings. Correct mistakes, identify inconsistencies, recommend exact website placements and crops, and flag anything that must not be claimed from the images alone.'
    }
  ];
  for (let index = 0; index < stages.length; index += 1) {
    ...
    try {
      ...
      addTeamMessage(project, { ...data.message, from: stage.role, to: index === 0 ? 'Visual Brand Reviewer' : 'Website Strategist', kind: index === 0 ? 'image-analysis' : 'image-review' });
    } catch (error) {
  ...
  intake.messages.push(intakeMessage('assistant', analyses.length
    ? `Picture scan complete. ${analyses.length} vision teammate${analyses.length === 1 ? '' : 's'} inspected the uploads. Their full findings are visible under Team, and every later AI will receive them.`
...
function inlinePreview(project, attachments = activeProject()?.attachments || []) {
...
async function buildWorkflow(project) {
  ...
  project.status = 'building'; project.artifact = null; project.previousArtifact = null; project.teamMessages = preflightMessages; project.attempts = preflightAttempts; project.completedModels = preflightModels; project.providers = preflightProviders; project.review = null; project.validation = null; project.changes = []; project.pendingTeamNotes = [];
  ...
  const build = await postProjectStep({ action: 'build', mode: project.mode, prompt: project.prompt, outputFormat: state.settings.outputFormat, modelIds, slot: activeRoles.length, workspace: project.teamMessages, projectName: project.title, profile: project.websiteProfile, concept: project.designConcept }, 50000);
  ...
  for (let round = 1; round <= state.settings.reviewRounds; round += 1) {
    ...
    const reviewData = await postProjectStep({ action: 'review', mode: project.mode, prompt: project.prompt, modelIds, slot: activeRoles.length + 1 + ((round - 1) * 2), workspace: project.teamMessages, project: artifactForAi(project.artifact), mediaPaths: (project.attachments || []).map((item) => item.path), profile: project.websiteProfile, concept: project.designConcept }, 50000);
    ...
    const repaired = await postProjectStep({ action: 'repair', mode: project.mode, prompt: project.prompt, modelIds, slot: activeRoles.length + 2 + ((round - 1) * 2), project: artifactForAi(project.artifact), review: reviewData.review, profile: project.websiteProfile, concept: project.designConcept, customInstruction: `Repair round ${round}. Resolve every concrete QA issue while preserving correct work and the approved design DNA.` }, 50000);
    recordAttempts(project, repaired.attempts || []); project.previousArtifact = before; project.artifact = materializeImageTokens(project, repaired.project); project.changes = computeChanges(before, project.artifact);
...
async function validateProject(project, modelIds = workflowModelIds(10), progress = 75) {
  ...
  const data = await postProjectStep({ action: 'validate', prompt: project.prompt, modelIds, slot: 9, project: artifactForAi(project.artifact), mediaPaths: (project.attachments || []).map((item) => item.path) }, 50000);
...
async function applyTeamNotes() {
  ...
  try {
    ...
    const result = await postProjectStep({ action: 'apply', prompt: project.prompt, modelIds, slot: 7, project: artifactForAi(project.artifact), review: { approved: false, summary: 'Apply latest team notes.', issues: [] }, customInstruction: instruction }, 50000);
...
async function downloadZip() {
  ...
  try {
    const response = await fetch('/api/package-project', { method: 'POST', headers: apiHeaders(true), body: JSON.stringify({ projectName: project.artifact.projectName, files: project.artifact.files, media: (project.attachments || []).map((item) => ({ path: item.path, contentBase64: String(item.dataUrl || '').split(',')[1] || '', mimeType: item.type })) }) });
...
els.buildForm.addEventListener('drop', (event) => { const files = [...(event.dataTransfer?.files || [])].filter((file) => file.type?.startsWith('image/')); if (files.length) { event.preventDefault(); addImageFiles(files); } });
...
els.openPreviewBtn.addEventListener('click', () => { const html = inlinePreview(activeProject()?.artifact, activeProject()?.attachments || []); if (!html) return; const url = URL.createObjectURL(new Blob([html], { type: 'text/html' })); window.open(url, '_blank', 'noopener'); setTimeout(() => URL.revokeObjectURL(url), 60000); });
