...
const STORAGE_KEY = 'omnifusion-v6-state';
...
async function callTeamStep(action, payload, timeoutMs = 70000) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  try {
    const response = await fetch('/api/team-step', {
      method: 'POST',
    ...
    const data = await readJsonOrText(response);
    if (!response.ok) {
      const error = new Error(data.error || `Collaboration step failed (${response.status}).`);
    ...
    if (error?.name === 'AbortError') throw new Error('A collaboration step took too long. The completed team messages were preserved.');
    throw error;
  } finally {
    clearTimeout(timer);
  }
...
function recordAttempts(attempts, tracker, scope = {}) {
  for (const attempt of attempts || []) {
    handleStreamEvent('provider_attempt', {
      ...scope,
    ...
    if (attempt.success) {
      tracker.callsCompleted += 1;
      if (attempt.actualModel) tracker.completedModels.add(attempt.actualModel);
      if (attempt.providerName) tracker.providers.add(attempt.providerName);
    } else {
      tracker.failedAttempts += 1;
      handleStreamEvent('provider_failure', {
        ...scope,
        providerId: attempt.providerId,
        providerName: attempt.providerName,
        requestedModel: attempt.requestedModel,
...
function recordCitations(items, tracker) {
  for (const item of items || []) if (item) tracker.citations.add(item);
}

function failedTeamMessage(role, to, kind, round, error) {
  return {
    id: uid('failed-message'),
    agentId: uid('failed-agent'),
    from: role.role,
    to,
    kind: `${kind}-failed`,
    round,
    content: `This teammate could not complete the handoff. The remaining team will continue using the shared transcript. Error: ${error.message}`,
    provider: 'No completed provider',
    providerId: 'none',
    model: 'step failed',
    latencyMs: 0,
...
async function executeTeamMessage({ prompt, modelIds, role, to, kind, round, slot, tracker }) {
  handleStreamEvent('team_message_start', {
    agentId: `agent-${slot + 1}`,
    from: role.role,
    to,
    kind,
    round,
    requestedModel: String(modelIds[slot % modelIds.length] || 'openrouter/free').replace(/^or:/, ''),
    ts: Date.now()
  });
  try {
    const data = await callTeamStep('message', {
      prompt,
      modelIds,
      role,
      to,
      ...
      slot,
      agentId: `agent-${slot + 1}`,
      workspace: state.teamMessages
    });
    const message = data.message;
    recordAttempts(message.attempts, tracker, { stage: 'team', role: role.role, round });
    recordCitations(message.citations, tracker);
    handleStreamEvent('team_message', message);
    return { ok: true, message };
  } catch (error) {
    recordAttempts(error.attempts, tracker, { stage: 'team', role: role.role, round });
    const message = failedTeamMessage(role, to, kind, round, error);
    handleStreamEvent('team_message', message);
    return { ok: false, message, error };
  }
}

async function runQuickWorkflow(prompt) {
  const started = Date.now();
  const tracker = createRunTracker();
  const modelIds = workflowModelIds(3);
  handleStreamEvent('run_start', { mode: 'quick', requestId: uid('run'), ts: started });
  handleStreamEvent('route_complete', { intent: 'auto', candidateCount: modelIds.length, ts: Date.now() });
  handleStreamEvent('agent_start', { agentId: 'primary', role: 'Primary AI', requestedModel: modelIds[0].replace(/^or:/, ''), ts: Date.now() });
  const data = await callTeamStep('quick', { prompt, modelIds, slot: 0 });
  recordAttempts(data.attempts, tracker, { stage: 'primary', role: 'Primary AI' });
  recordCitations(data.citations, tracker);
  handleStreamEvent('agent_complete', { agentId: 'primary', role: 'Primary AI', provider: data.provider, model: data.model, latencyMs: data.latencyMs, ts: Date.now() });
  const models = [...tracker.completedModels];
  return {
    answer: data.answer,
    goal: 'Direct answer from one available model.',
    intent: data.intent || 'general',
    teamMessages: [],
    agents: [{ role: 'Primary AI', content: data.answer, provider: data.provider, model: data.model, latencyMs: data.latencyMs, attempts: data.attempts || [] }],
    finalizer: null,
    providersUsed: [...tracker.providers],
    completedModels: models,
    distinctModels: models.length,
    callsCompleted: tracker.callsCompleted,
...
async function runCollaborativeWorkflow(prompt) {
  const started = Date.now();
  const tracker = createRunTracker();
  const agentCount = state.mode === 'deep' ? 5 : 4;
  const rounds = state.mode === 'deep' ? Math.max(2, state.collaborationRounds) : state.collaborationRounds;
  const modelIds = workflowModelIds(Math.max(agentCount + 2, 7));
  const requestId = uid('team-run');

  handleStreamEvent('run_start', { mode: state.mode, requestId, collaborationRounds: rounds, ts: started });
  handleStreamEvent('route_complete', { intent: 'auto', candidateCount: modelIds.length, requestedAgents: agentCount, ts: Date.now() });
  handleStreamEvent('planner_start', { requestedModel: modelIds[0].replace(/^or:/, ''), ts: Date.now() });

  const plan = await callTeamStep('plan', { prompt, modelIds, agentCount, slot: 0 });
  recordAttempts(plan.planner?.attempts, tracker, { stage: 'planner' });
  recordCitations(plan.planner?.citations, tracker);
  const roles = Array.isArray(plan.roles) ? plan.roles : [];
  if (roles.length < 3) throw new Error('The team planner did not return enough connected roles.');
  const goal = plan.goal || 'Complete the request through visible collaboration.';

  handleStreamEvent('planner_complete', {
    provider: plan.planner?.provider,
    model: plan.planner?.model,
    latencyMs: plan.planner?.latencyMs,
    roles: roles.map((role) => role.role),
    ts: Date.now()
  });
  handleStreamEvent('collaboration_start', { goal, roles: roles.map((role) => role.role), rounds, ts: Date.now() });

  const lead = roles[0];
  const reviewer = roles[roles.length - 1];
  const specialists = roles.slice(1, -1);

  handleStreamEvent('round_start', { round: 0, label: 'Team lead planning', ts: Date.now() });
  await executeTeamMessage({
    prompt,
    modelIds,
    role: lead,
    to: specialists[0]?.role || reviewer.role,
    kind: 'plan',
    round: 0,
    slot: 0,
    tracker
  });
  handleStreamEvent('round_complete', { round: 0, completed: 1, ts: Date.now() });

  handleStreamEvent('round_start', { round: 1, label: 'Sequential specialist collaboration', ts: Date.now() });
  let specialistCompleted = 0;
  for (let index = 0; index < specialists.length; index += 1) {
    const role = specialists[index];
    const next = specialists[index + 1]?.role || reviewer.role;
    const result = await executeTeamMessage({
      prompt,
      modelIds,
      role,
      to: next,
    ...
    if (result.ok) specialistCompleted += 1;
  }
  handleStreamEvent('round_complete', { round: 1, completed: specialistCompleted, ts: Date.now() });

  let successfulReviews = 0;
  let successfulRevisions = 0;
  for (let reviewRound = 1; reviewRound <= rounds; reviewRound += 1) {
    const round = reviewRound + 1;
    handleStreamEvent('round_start', { round, label: `Review and revision ${reviewRound}`, ts: Date.now() });
    const reviewResult = await executeTeamMessage({
      prompt,
      modelIds,
      role: reviewer,
      to: lead.role,
      kind: reviewRound === rounds ? 'quality-review' : 'review',
      round,
      slot: roles.length - 1,
      tracker
    });
    if (reviewResult.ok) successfulReviews += 1;

    const revisionRole = {
      ...lead,
      instruction: `${lead.instruction} Read the reviewer and every specialist. Reply directly to the reviewer, resolve each concrete issue, reconcile disagreements, and update the shared deliverable.`,
      deliverable: reviewRound === rounds
        ? 'A revised near-final deliverable that explicitly resolves the review.'
        : 'A revised draft and a direct response to the reviewer.'
    };
    const revisionResult = await executeTeamMessage({
      prompt,
      modelIds,
      role: revisionRole,
      to: reviewRound === rounds ? 'Final Integrator' : reviewer.role,
      kind: 'revision',
      round,
      slot: 0,
      tracker
    });
    if (revisionResult.ok) successfulRevisions += 1;
    handleStreamEvent('round_complete', { round, completed: Number(reviewResult.ok) + Number(revisionResult.ok), ts: Date.now() });
  }

  handleStreamEvent('finalizer_start', {
    requestedModel: String(modelIds[roles.length % modelIds.length] || modelIds[0]).replace(/^or:/, ''),
    messageCount: state.teamMessages.length,
    ts: Date.now()
  });

  let finalData = null;
  try {
    finalData = await callTeamStep('finalize', {
      prompt,
      modelIds,
      goal,
      workspace: state.teamMessages,
    ...
    recordAttempts(finalData.attempts, tracker, { stage: 'finalizer' });
    recordCitations(finalData.citations, tracker);
    handleStreamEvent('finalizer_complete', { provider: finalData.provider, model: finalData.model, latencyMs: finalData.latencyMs, ts: Date.now() });
  } catch (error) {
    recordAttempts(error.attempts, tracker, { stage: 'finalizer' });
    handleStreamEvent('provider_failure', { stage: 'finalizer', providerName: 'Final Integrator', error: error.message, ts: Date.now() });
  }

  const successfulMessages = state.teamMessages.filter((message) => !message.failed);
  const fallbackAnswer = [...successfulMessages].reverse().find((message) => message.kind === 'revision')?.content
    || successfulMessages.at(-1)?.content
    || 'The collaboration could not produce a final deliverable.';
  const completedModels = [...tracker.completedModels];
  const collaborationVerified = successfulMessages.length >= 3 && successfulReviews > 0 && successfulRevisions > 0;
  const verifiedMultiModel = collaborationVerified && completedModels.length >= 2;
  const agents = successfulMessages.map((message) => ({
    role: message.from,
  ...
  return {
    answer: finalData?.answer || fallbackAnswer,
    goal,
    intent: plan.intent || 'general',
    roles: roles.map((role) => role.role),
    teamMessages: state.teamMessages,
    agents,
    finalizer: finalData ? { provider: finalData.provider, model: finalData.model, latencyMs: finalData.latencyMs, attempts: finalData.attempts || [] } : null,
    providersUsed: [...tracker.providers],
    completedModels,
...
async function runPrompt(event) {
  ...
  conversation.messages.push({ role: 'assistant', content: '', pending: true, status: 'Starting the collaboration…' });
  conversation.updatedAt = new Date().toISOString();
  els.prompt.value = '';
  autoResizePrompt();
  updateCharCount();
  ...
  try {
    const payload = state.mode === 'quick'
      ? await runQuickWorkflow(prompt)
      : await runCollaborativeWorkflow(prompt);
    state.finalPayload = payload;
    handleStreamEvent('run_complete', { ...payload, ts: Date.now() });
    finishRun(payload);
  } catch (error) {
    ...
    if (pendingIndex >= 0) {
      const lastUseful = [...state.teamMessages].reverse().find((item) => !item.failed && item.content);
      conversation.messages[pendingIndex] = {
        role: 'assistant',
        content: lastUseful
          ? `**The run stopped before final integration. Completed team work was preserved below.**\n\n${lastUseful.content}`
          : `**Run error:** ${message}`,
        meta: {
          distinctModels: 0,
          models: [],
    ...
    saveState();
    renderMessages();
    renderTeamRoom(state.teamMessages, { collaborationVerified: false });
    showError(message);
