...
async function loadStatus() {
  try {
    ...
    state.status = data; document.title = data.appName || 'OmniFusion Motion Studio V17';
...
// V17 motion layer: animates real interface changes without changing workflow behavior.
const motionQuery = window.matchMedia('(prefers-reduced-motion: reduce)');
const motionTargets = '.chat-message,.inline-question-card,.inline-answer,.inline-concept,.brief-ready-card,.build-chat-card,.release-chat-card,.team-message,.file-row,.file-card,.change-card,.validation-card,.memory-card,.version-card,.intel-card,.quality-summary-card,.quality-dashboard,.model-card,.integration-card,.attachment-card';

function markMotionTargets(root = document) {
  if (motionQuery.matches) return;
  const nodes = [];
  if (root instanceof Element && root.matches(motionTargets)) nodes.push(root);
  if (root.querySelectorAll) nodes.push(...root.querySelectorAll(motionTargets));
  nodes.slice(-28).forEach((node, index) => {
    if (node.dataset.motionSeen === '1') return;
    node.dataset.motionSeen = '1';
    node.style.setProperty('--enter-order', String(index));
...
function installMotionLayer() {
  requestAnimationFrame(() => {
    ...
    markMotionTargets(document);
  });
  const observer = new MutationObserver((records) => {
    for (const record of records) for (const node of record.addedNodes) if (node.nodeType === Node.ELEMENT_NODE) markMotionTargets(node);
  });
  observer.observe(document.body, { childList: true, subtree: true });

  let pointerFrame = 0;
  document.addEventListener('pointermove', (event) => {
    if (motionQuery.matches || pointerFrame) return;
    pointerFrame = requestAnimationFrame(() => {
      pointerFrame = 0;
      document.documentElement.style.setProperty('--mouse-x', `${Math.round((event.clientX / innerWidth) * 100)}%`);
      document.documentElement.style.setProperty('--mouse-y', `${Math.round((event.clientY / innerHeight) * 100)}%`);
    });
  }, { passive: true });
