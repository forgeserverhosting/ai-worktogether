...
test('one OpenRouter key loads models and runs a collaborative multi-model team', async () => {
    ...
    if (href.endsWith('/api/v1/chat/completions')) {
      ...
      if (system.includes('Return JSON only')) {
        content = JSON.stringify({ goal: 'Use several OpenRouter models.', roles: [
          { role: 'Team Lead', instruction: 'Plan it.', deliverable: 'Plan' },
          { role: 'Builder', instruction: 'Build it.', deliverable: 'Draft' },
          { role: 'Quality Reviewer', instruction: 'Review it.', deliverable: 'Review' }
        ] });
  ...
  await orchestrateHandler({ method: 'POST', headers: { 'x-forwarded-for': `or-test-${Date.now()}` }, body: { prompt: 'Build a website', mode: 'team', agentCount: 3, collaborationRounds: 1 }, socket: {} }, orchestrationRes);
  assert.equal(orchestrationRes.code, 200);
  assert.equal(orchestrationRes.payload.answer, 'Combined multi-model answer.');
  assert.ok(orchestrationRes.payload.teamMessages.length >= 4);
  assert.equal(orchestrationRes.payload.collaborationVerified, true);
  assert.ok(new Set(orchestrationRes.payload.teamMessages.map((message) => message.model)).size >= 2);
