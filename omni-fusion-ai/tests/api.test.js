...
test('full council runs through an OpenAI-compatible custom provider', async () => {
  const server = http.createServer(async (req, res) => {
    ...
    if (system.includes('Return JSON only')) {
      content = JSON.stringify({ goal: 'Test the complete council.', roles: [
        { role: 'Planner', instruction: 'Create the plan.' },
        { role: 'Reviewer', instruction: 'Review the work.' },
        { role: 'Editor', instruction: 'Improve the result.' }
      ] });
  ...
  const res = responseMock();
  await orchestrateHandler({ method: 'POST', headers: { 'x-forwarded-for': `test-${Date.now()}` }, body: { prompt: 'Create a test answer', mode: 'council' }, socket: {} }, res);
  assert.equal(res.code, 200);
  assert.equal(res.payload.answer, 'Final synthesized answer.');
  assert.equal(res.payload.agents.length, 3);
