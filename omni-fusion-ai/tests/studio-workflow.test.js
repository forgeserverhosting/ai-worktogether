...
test('development studio builds, reviews, repairs, and validates real files', async () => {
  const server = http.createServer(async (req, res) => {
    ...
    let content;
    if (system.includes('world-class web Creative Director')) {
      content = JSON.stringify({ concepts: [
        { id: 'concept-a', name: 'Concept A', tagline: 'A', palette: ['#111111', '#eeeeee', '#ff7700'], layout: 'Asymmetric editorial', motion: 'Smooth reveals', signature: 'Framed corners', why: 'Fits' },
        { id: 'concept-b', name: 'Concept B', tagline: 'B', palette: ['#eeeeee', '#222222', '#bb5533'], layout: 'Field journal', motion: 'Tactile transitions', signature: 'Project stamps', why: 'Fits' },
        { id: 'concept-c', name: 'Concept C', tagline: 'C', palette: ['#080808', '#d8ff4f', '#ffffff'], layout: 'Modular canvas', motion: 'Kinetic type', signature: 'Living service map', why: 'Fits' }
      ] });
    } else if (system.includes('Build actual project files')) {
  ...
  await projectStepHandler({ ...reqBase, body: { action: 'concepts', prompt: 'Build a demo website', profile: { personality: 'Bold' } } }, res);
  assert.equal(res.code, 200);
  assert.equal(res.payload.concepts.length, 3);

  res = responseMock();
  await projectStepHandler({ ...reqBase, headers: { 'x-forwarded-for': `studio-build-${Date.now()}` }, body: { action: 'build', prompt: 'Build a demo', outputFormat: 'static' } }, res);
  assert.equal(res.code, 200);
