  ...
  try {
    ...
    if (action === 'repair' || action === 'apply') {
      ...
      const result = await callWithinBudget(providers, [
        {
          role: 'system',
          content: `You are the Fixer Developer. Modify the ACTUAL project files to resolve the review and user instruction. ${contractInstruction('project', 'Fixer Developer')} The payload must be {"projectName":"...","projectType":"static|single-html|nextjs","entryFile":"...","summary":"...","files":[{"path":"...","content":"complete updated content"}],"notes":["..."]}. Include every file that should remain in the project, even unchanged files. Do not return patches, prose, or markdown. ${projectMode === 'website' ? 'Preserve or strengthen the approved design DNA and remove generic template patterns.' : 'Preserve the correct shared project decisions.'}`
        },
        {
          role: 'user',
