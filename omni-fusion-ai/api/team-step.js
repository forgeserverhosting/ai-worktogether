...
export default async function handler(req, res) {
  ...
  try {
    ...
    const memoryContext = relevantMemoryText(body.memoryContext || {}, cleanString(body.memoryRole, 100) || 'AI teammate', 10000);
    ...
    const requestedRole = cleanString(body.role?.role || body.role, 80).toLowerCase();
    const taskRole = action === 'vision' ? 'vision'
      : action === 'quick' ? 'quick'
        : action === 'plan' || action === 'finalize' ? 'lead'
          : /review|critic|quality|verify|test/.test(requestedRole) ? 'reviewer'
            : /design|creative|copy|writer/.test(requestedRole) ? 'creative'
              : /developer|engineer|architect|code/.test(requestedRole) ? 'builder'
                : 'lead';
    const roster = await resolvePrimeProviders({ modelIds, taskRole, prompt, desiredCount: 4, excludeIds: body.excludeModelIds });
    ...
    if (action === 'vision') {
      const images = (Array.isArray(body.images) ? body.images : []).slice(0, 4).flatMap((item, index) => {
        ...
        return [{ id: cleanString(item?.id, 80) || `image-${index + 1}`, name: cleanString(item?.name, 100) || `Picture ${index + 1}`, role: item?.role === 'reference' ? 'reference' : 'content', dataUrl }];
      });
...
${contractInstruction('handoff', role.role)} The payload must be {"message":"the visible handoff to the next teammate","evidence":["specific fact, decision, file, or test supporting the handoff"],"filesReceived":["file or input actually received"],"filesChanged":["file actually changed, if any"],"tests":["test actually performed, if any"]}. Read the shared transcript. Respond to prior teammates, preserve useful decisions, correct problems you notice, and make a concrete handoff to ${to}. Do not act like an isolated contestant. Share conclusions, edits, decisions, evidence, questions, code, and usable artifacts—but not hidden private chain-of-thought.`
        },
        {
          role: 'user',
...
${memoryContext || '(No saved project memory.)'}
...
YOUR DELIVERABLE:\n${role.deliverable}\n\nSHARED TEAM TRANSCRIPT:\n${prior || '(No earlier messages. Start the project and hand off clearly.)'}\n\nWrite your visible team message to ${to}.`
      ...
      const contract = normalizeAgentContract(parsed, 'handoff');
      const contractValidation = validateAgentContract(contract, 'handoff');
      ...
      const message = {
        ...
        kind,
        round,
        content: cleanString(payload.message, 12000) || contract.summary || result.content,
        evidence: Array.isArray(payload.evidence) ? payload.evidence.slice(0, 12) : contract.evidence,
        filesReceived: Array.isArray(payload.filesReceived) ? payload.filesReceived.slice(0, 16) : [],
        filesChanged: Array.isArray(payload.filesChanged) ? payload.filesChanged.slice(0, 16) : [],
        tests: Array.isArray(payload.tests) ? payload.tests.slice(0, 16) : [],
        contractStatus: contract.status,
        contractValid: contractValidation.ok,
        contractVersion: contract.contractVersion,
        ...resultMeta(result),
        createdAt: Date.now()
      };
      return sendJson(res, 200, { requestId, action, message, contract, ...contractMeta(contract, contractValidation) });
    }
