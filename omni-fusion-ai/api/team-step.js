...
import { inferIntent, fallbackRoles, parseJsonObject } from './lib/routing.js';
...
async function callWithinBudget(providers, messages, options = {}) {
  const attempts = [];
  const totalBudgetMs = clampInt(options.totalBudgetMs, 15000, 54000, 52000);
  const maxAttemptMs = clampInt(options.maxAttemptMs, 8000, 46000, 34000);
