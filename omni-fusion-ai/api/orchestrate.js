...
export default async function handler(req, res) {
  ...
  const available = await resolveProviders({ intent, requestedIds, desiredCount: Math.max(requestedAgents + 2, 5) });
  if (!available.length) {
    return sendJson(res, 503, { error: 'No AI provider is configured. Add OPENROUTER_API_KEY in Vercel.' });
  }
