...
async function callOpenAICompatible(provider, messages, options) {
  ...
  const supported = Array.isArray(provider.supportedParameters) ? new Set(provider.supportedParameters) : null;
  if (!supported || supported.has('temperature')) body.temperature = options.temperature;
  if (!supported || supported.has('max_tokens')) body.max_tokens = options.maxTokens;
...
export async function callWithFallback(providers, messages, options = {}) {
  ...
  for (const provider of providers) {
    ...
    onAttempt?.({ phase: 'start', providerId: provider.id, providerName: provider.name, requestedModel: provider.model, ts: attemptStarted });
    try {
      ...
      onAttempt?.({ phase: 'success', ...attempt, ts: Date.now() });
      ...
      attempts.push(attempt);
      onAttempt?.({ phase: 'failure', ...attempt, ts: Date.now() });
    }
  }
  const summary = attempts.map((item) => `${item.providerName}: ${item.error}`).join(' | ');
  const error = new Error(`No provider completed the request. ${summary}`);
