...
export function scoreOpenRouterModel(model, { intent = { primary: 'general' }, taskRole = 'general', allowPaid = openRouterAllowPaid() } = {}) {
...
export function selectOpenRouterModels(models, { intent, taskRole = 'general', count = 5, selectedIds = [], allowPaid = openRouterAllowPaid(), excludeIds = [] } = {}) {
  ...
  const ranked = rankOpenRouterModels(models, { intent, taskRole, allowPaid })
    .map((item) => item.model)
